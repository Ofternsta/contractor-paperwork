...
export function MarketingHome() {
  return (
    <div className="min-h-dvh flex flex-col bg-background text-foreground">
      <header className="marketing-header sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-md safe-top">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="hidden sm:flex items-center gap-6 text-sm font-medium text-muted">
            <a href="#features" className="hover:text-brand-bright transition-colors duration-200">
              Features
            </a>
            <a href="#how-it-works" className="hover:text-brand-bright transition-colors duration-200">
              How it works
            </a>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              href="/login"
              className="text-sm font-medium text-muted px-3 py-2 min-h-[44px] inline-flex items-center hover:text-foreground transition-colors duration-200"
            >
              Sign in
            </Link>
            <Link
      ...
      <main className="flex-1">
        <section className="relative overflow-hidden border-b border-border">
          <div
            className="marketing-hero-glow absolute inset-0 pointer-events-none"
            style={{
              background:
            ...
            <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-12 xl:gap-16">
              ...
              <div className="hidden lg:flex shrink-0 justify-center items-center order-2">
                <BrandLogo
                  ...
                  size="hero-xl"
                  priority
                  className="marketing-logo-float drop-shadow-[0_0_56px_var(--brand-glow)]"
                />
              </div>
            </div>
          </div>
        ...
        >
          ...
          <ul className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {(
              ...
              <Reveal key={id} as="li" delay={i * 70} className="h-full">
                <div
                  className={`rounded-2xl p-5 flex flex-col border h-full ${
                    id === 'professional'
                      ? 'marketing-pricing-popular border-brand ring-2 ring-brand/40 shadow-[0_0_24px_var(--brand-glow)] bg-surface-elevated'
                      : 'marketing-card-lift border-border bg-surface'
                  }`}
                >
                  {id === 'professional' && (
      ...
      <footer className="border-t border-border bg-background">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-muted">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-4 sm:gap-6">
            ...
            <Link href="/terms" className="hover:text-brand-bright transition-colors duration-200">
              Terms
            </Link>
            <Link href="/login" className="hover:text-brand-bright transition-colors duration-200">
              Sign in
            </Link>
            <Link href="/login?signup=admin" className="hover:text-brand-bright transition-colors duration-200">
              Sign up
            </Link>
          </div>
        </div>
